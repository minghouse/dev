...
const getDatas8 = async (search_date, search_date2) => { 
    ...
    for (const k in sheetData) {
        if ([ "AI整理-經濟日報", "AI整理-yahoo財經", "AI整理-工商時報", "AI整理-時報新聞" ].includes(sheetData_name[k])) {
    ...
    const result = await (async ()=>{
        ...
        new_right_data.sort(function(a,b){
            if (isNaN(a[4]) || !a[4]) a[5] = 0
            if (isNaN(b[4]) || !b[4]) b[5] = 0
            if (isNaN(a[5]) || !a[5]) a[5] = 0
            if (isNaN(b[5]) || !b[5]) b[5] = 0
